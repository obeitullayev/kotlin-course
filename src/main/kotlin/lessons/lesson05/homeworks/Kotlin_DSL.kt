...
import lessons.lesson04.homework.Step
...
// Расписание состоит из этих пунктов, разбитых по дням недели без привязки к дате - каждую неделю одно и то же расписание.
//
//Твоя задача - дополнить класс Schedule методами, реализующими DSL, необходимый для удобного заполнения данными.
...
//Для передачи нужного дня недели используй внешнюю относительно метода переменную var в которую нужно сохранить используемый
// в текущем блоке день недели и потом обнулить по завершении блока.
...
class Schedule {
    ...
    operator fun String.rangeTo(time: String): Pair<LocalTime, LocalTime>{
        return LocalTime.parse(this, timeFormatter) to LocalTime.parse(time, timeFormatter)
    }

    infix fun Pair<LocalTime, LocalTime>.schedule(lesson: String){
        addSchedule(day?: throw IllegalStateException("Не задан день"), ScheduleEntity(lesson, first, second) )
    }
